...
func TestPackageStore(t *testing.T) {
	...
	// Test GetUnquarantined + Unquarantine
	...
	// Test GetOrInsertPending(Assign)
	pkg5, err := s.Package.GetOrInsertPending(purl2, pathname)
	assert.NoError(t, err)
	assert.Equal(t, pathname, pkg5.Pathname)
	assert.Equal(t, pkg3.ID, pkg5.ID)
	assert.Equal(t, entity.Unquarantined, pkg5.State)

	pkg5, err = s.Package.GetOrInsertPending(purl2, "")
	assert.NoError(t, err)
	assert.Equal(t, pathname, pkg5.Pathname)
	assert.Equal(t, pkg3.ID, pkg5.ID)
	assert.Equal(t, entity.Unquarantined, pkg5.State)

	// Test GetOrInsertPending(Attr)
	pkg6, err := s.Package.GetOrInsertPending(purl3, pathname)
	assert.NoError(t, err)
	assert.Equal(t, purl3, pkg6.Purl)
	assert.Equal(t, pathname, pkg6.Pathname)
	assert.Equal(t, entity.Pending, pkg6.State)
}
